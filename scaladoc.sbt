...
apiMappings ++= {
  ...
  Map(
    ...
    findManagedDependency("com.github.ben-manes.caffeine", "caffeine") -> url("http://static.javadoc.io/com.github.ben-manes.caffeine/caffeine/2.2.3/")
  )
}
...
fixJavaLinksTask := {
  ...
  val caffeineApiLink = """\"(http://static\.javadoc\.io/com\.github\.ben-manes\.caffeine/caffeine/2\.2\.3/)index\.html#([^"]*)\"""".r
  ...
  (t ** "*.html").get.filter(hasJavadocLink).foreach { f =>
    try {
      log.info("Fixing " + f)
      val cfixedContent = replaceCaffeineLinks.apply(IO.read(f))
      val jfixedContent = replaceJdkLinks.apply(cfixedContent)
      IO.write(f, jfixedContent)
    } catch {
